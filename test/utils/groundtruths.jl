...
function _groundtruth_unsafe_probability(proc, in_ps::AbstractVector, out_ps::AbstractMatrix)
...
function _groundtruth_unsafe_probability(proc, in_ps::AbstractMatrix, out_ps::AbstractVector)
...
function _groundtruth_unsafe_probability(proc, in_ps::AbstractMatrix, out_ps::AbstractMatrix)
    res = Matrix{Float64}(undef,size(in_ps,2),size(out_ps,2))
    for i in 1:size(in_ps,2)
        for j in 1:size(out_ps,2)
            res[i,j] = _groundtruth_unsafe_probability(proc,view(in_ps,:,i),view(out_ps,:,j))
        end
    end
    return res
end
...
function _groundtruth_safe_probability(proc, in_ps::AbstractMatrix, out_ps::AbstractMatrix)
    ...
    for i in 1:size(in_ps,2)
        for j in 1:size(out_ps,2)
            res[i,j] = _groundtruth_safe_probability(proc,view(in_ps,:,i),view(out_ps,:,j))
        end
    end
    return res
end
...
function _groundtruth_unsafe_diffCS(proc, in_ps::AbstractMatrix, out_ps::AbstractMatrix)
    ...
    for i in 1:size(in_ps,2)
        for j in 1:size(out_ps,2)
            res[i,j] = _groundtruth_unsafe_diffCS(proc,view(in_ps,:,i),view(out_ps,:,j))
        end
    end
    return res
end
...
function _groundtruth_safe_diffCS(proc, in_ps::AbstractMatrix, out_ps::AbstractMatrix)
    ...
    for i in 1:size(in_ps,2)
        for j in 1:size(out_ps,2)
            res[i,j] = _groundtruth_safe_diffCS(proc,view(in_ps,:,i),view(out_ps,:,j))
        end
    end
    return res
end
